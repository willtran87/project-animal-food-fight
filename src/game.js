(() => {
  ...
  const ACTIVE_RUN_AUTOSAVE_SECONDS = 5;
  const RUN_LOG_LIMIT = 120;
  ...
  const IDLE_UPDATE_INTERVAL_MS = 250;
  ...
  function accessibleUnitNames(units) {
    return (units || [])
      .filter((unit) => isUnit(unit) && !unit.dead)
      .map((unit) => displayUnitShort(unit))
      .join(", ");
  }

  function accessibleStatusText() {
    const parts = [
      `Round ${state.round}.`,
      `Phase ${state.phase}.`,
      `${state.gold} ${currencyTerm({ lower: true })}.`,
      `${state.hearts} ${realityBroken() ? "hull" : "hearts"}.`,
    ];
    if (state.message) parts.push(`${state.message}.`);
    if (state.phase === "prep") {
      const team = accessibleUnitNames(state.board);
      const shop = (state.shop || []).filter(Boolean).map((entry) => entryLabel(entry)).join(", ");
      parts.push(team ? `Team: ${team}.` : "Team is empty.");
      if (shop) parts.push(`Shop: ${shop}.`);
    } else if (state.phase === "battle" && state.battle) {
      const allies = state.battle.allies.filter((unit) => !unit.dead).length;
      const enemies = state.battle.enemies.filter((unit) => !unit.dead).length;
      parts.push(`${allies} allies and ${enemies} enemies remain.`);
    }
    if (state.activeStory) {
      const beat = state.activeStory.beats?.[state.activeStory.index];
      if (beat?.speaker || beat?.text) parts.push(`${beat.speaker || "Narrator"}: ${beat.text || ""}`);
    }
    if (state.optionsMenu?.open) parts.push("Options menu open.");
    if (state.codexOpen) parts.push("Codex open.");
    return parts.join(" ");
  }

  function syncAccessibleStatus(force = false) {
    if (!accessibleStatus) return;
    const now = performance.now();
    const phaseKey = `${state.phase}|${state.round}|${state.message}|${Boolean(state.optionsMenu?.open)}|${Boolean(state.codexOpen)}`;
    if (!force && phaseKey === lastAccessiblePhaseKey && now - lastAccessibleStatusAt < 1000) return;
    const next = accessibleStatusText();
    lastAccessiblePhaseKey = phaseKey;
    lastAccessibleStatusAt = now;
    if (next === lastAccessibleStatus) return;
    lastAccessibleStatus = next;
    accessibleStatus.textContent = next;
  }
  ...
  function addRunLog(...entries) {
    const values = entries.filter((entry) => typeof entry === "string" && entry.length > 0);
    if (!values.length) return;
    state.log.unshift(...values);
    if (state.log.length > RUN_LOG_LIMIT) state.log.length = RUN_LOG_LIMIT;
  }
  const gameSfx = {
    armed: false,
    volumeSetting: null,
  ...
  function runSnapshotFingerprint(snapshot) {
  ...
  function saveCurrentRun(options = {}) {
    ...
    const fingerprint = runSnapshotFingerprint(snapshot);
    if (options.silent && fingerprint === lastSilentSnapshotFingerprint) return true;
    const saved = writeActiveRunRecord(snapshot);
    if (saved) {
      lastSilentSnapshotFingerprint = fingerprint;
  ...
  function triggerRealityBreak() {
    ...
    addRunLog(copy("ui.reality.triggerLog", "Illusion failed: future war layer exposed"));
  ...
  function combatLedgerTeamStats(ledger) {
    ...
    const allyStatusDamage = Number.isFinite(ledger.ally?.statusDamageDealt)
      ? ledger.ally.statusDamageDealt
      : events.reduce((total, event) => (
          event.type === "damage" && event.kind === "status" && ledgerUnitSide(ledger, event.sourceUid) === "ally"
            ? total + (event.amount || 0)
            : total
        ), 0);
    const shieldAbsorbed = Number.isFinite(ledger.ally?.shieldAbsorbed)
      ? ledger.ally.shieldAbsorbed
      : events.reduce((total, event) => (
  ...
  function applyPendingArenaRewardBuff(units) {
    ...
    if (target) {
      ...
      addRunLog(`${target.short} carried ${buff.arenaShort} prep`);
    }
  ...
  function purchaseShopSlot(index) {
    ...
    addRunLog(`${realityBroken() ? "Opened scan slot" : "Opened shop slot"} ${index + 1}`);
    playGameSfx("upgrade");
    return true;
  }
  ...
  function upgradeShop() {
    ...
    addRunLog(`${upgradeTerm()} ${realityBroken() ? "rig" : "shop"} to level ${state.shopLevel}`);
  ...
  function mergeItemTriples(itemId, tier) {
    ...
    return startMergeCutscene({
      ...
      addRunLog(`${evolved.name} reached ${itemLevelLabel(evolved)}${reward ? ` and earned ${reward} ${currencyTerm({ lower: true })}` : ""}`);
      playGameSfx("merge");
    });
  }
  ...
  function buyShopMergeIntoSlot(shopIndex, targetArea, targetIndex) {
    ...
    return startMergeCutscene({
      ...
      addRunLog(`${evolved.name} reached ${evolved.tier} stars${reward ? ` and earned ${reward} ${currencyTerm({ lower: true })}` : ""}`);
      playGameSfx("merge");
    });
  }
  ...
  function mergeTriples(typeId, tier) {
    ...
    return startMergeCutscene({
      ...
      addRunLog(`${evolved.name} reached ${tier + 1} stars${reward ? ` and earned ${reward} ${currencyTerm({ lower: true })}` : ""}`);
      playGameSfx("merge");
    });
  }
  ...
  function maybeStartStoryMilestone() {
    ...
    if (milestone.log) addRunLog(milestone.log);
    return true;
  }
  ...
  function startFinalTabsStoryConversation() {
    ...
    if (FINAL_TABS_STORY.log) addRunLog(FINAL_TABS_STORY.log);
    return true;
  }
  ...
  function startLevel10RevealCutscene(options = {}) {
    ...
    addRunLog("Cutscene: level 10 reveal aftermath");
  ...
  const combatFoeScratch = [];

  function appendLivingUnits(target, units) {
  ...
  function updateBattleStatuses(battle, dt) {
    combatUnitScratch.length = 0;
    combatUnitScratch.push(...battle.allies, ...battle.enemies);
    combatUnitScratch.forEach((unit) => {
      if (unit.dead) return;
  ...
  function scheduleGameLoop(continuous = shouldRenderContinuously()) {
    if (frameRequestPending || idleLoopTimer) return;
    if (continuous) {
  ...
  function wakeGameLoop() {
    if (idleLoopTimer) {
      window.clearTimeout(idleLoopTimer);
      idleLoopTimer = null;
    }
    scheduleGameLoop(true);
  }

  function gameLoop(now) {
    frameRequestPending = false;
    const wasContinuous = shouldRenderContinuously();
    const maxDt = wasContinuous ? 0.05 : IDLE_UPDATE_INTERVAL_MS / 1000;
    const dt = Math.min(maxDt, ((now || 0) - state.lastTime) / 1000 || 1 / 60);
  ...
  function applyStoryConversationRoute(routeId) {
    ...
    if (story.log) addRunLog(story.log);
    startStoryConversation({
      id: story.id,
      title: story.title,
      index: 0,
  ...
  function compactUnitText(unit) {
    ...
    return {
      ...
      statuses: activeStatusEffects(unit).map((effect) => effect.id),
    };
  }

  function compactEntryText(entry) {
    if (!entry) return null;
    return {
      kind: isItem(entry) ? "item" : "unit",
      id: entry.id || entry.typeId,
  ...
  function renderCompactGameToText() {
    const battle = visibleBattle();
    const storyBeat = state.activeStory?.beats?.[state.activeStory.index] || null;
    return JSON.stringify({
      coordinateSystem: "Logical canvas 1024x640; origin top-left; x right; y down.",
      runMode: state.runMode,
      ...
      arena: arenaText(),
      traits: Object.fromEntries(Object.entries(TRAITS).map(([id, trait]) => [id, { label: traitDisplayText(id) || trait.label || id }])),
      rendering: {
        dirty: renderDirty,
        continuous: shouldRenderContinuously(),
        drawCount,
      ...
      overlays: {
        ...
        combatLedger: Boolean(state.combatLedgerReview?.open),
      },
      story: state.activeStory ? {
        id: state.activeStory.id,
        title: state.activeStory.title,
        index: state.activeStory.index,
        total: state.activeStory.beats?.length || 0,
        speaker: storyBeat?.speaker || null,
        text: storyBeat?.text || null,
      } : null,
      shop: state.shop.map(compactEntryText),
      bench: state.bench.map(compactEntryText),
      itemBench: state.itemBench.map(compactEntryText),
      board: state.board.map(compactEntryText),
      drinks: state.drinks.map(compactEntryText),
      battle: battle ? {
        elapsed: Number((battle.elapsed || 0).toFixed(2)),
        result: battle.result || null,
        allies: battle.allies.map(compactUnitText),
        enemies: battle.enemies.map(compactUnitText),
      } : null,
      lastCombat: state.lastCombatLedger ? combatLedgerTeamStats(state.lastCombatLedger) : null,
    });
  }

  function renderGameToText(options = {}) {
    return options?.detail === "full" ? renderFullGameToText() : renderCompactGameToText();
  }

  window.render_game_to_text = renderGameToText;
  window.render_game_to_text_full = renderFullGameToText;
  window.advanceTime = (ms, options = {}) => {
    const steps = Math.max(1, Math.round(ms / (1000 / 60)));
    for (let i = 0; i < steps; i++) update(1 / 60);
    drawFrame();
    return options.renderState ? renderGameToText() : undefined;
  };
